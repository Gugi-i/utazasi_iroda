...
def cancel_journey(journey_id: int, db: Session = Depends(get_db)):
...
def add_car(journey_id: int, data: AddCar, db: Session = Depends(get_db)):
    return journey_crud.add_car_to_journey(db, journey_id, data.car_rented_id)


@router.post("/{journey_id}/add_plane")
def add_plane(journey_id: int, data: AddPlane, db: Session = Depends(get_db)):
...
def add_accommodation(journey_id: int, data: AddAccommodation, db: Session = Depends(get_db)):
    return journey_crud.add_accommodation_to_journey(db, journey_id, data.accommodation_booked_id)


# ---- REMOVE ELEMENTS ----
@router.delete("/cars/{item_id}")
def delete_car(item_id: int, db: Session = Depends(get_db)):
...
def delete_plane(item_id: int, db: Session = Depends(get_db)):
...
def delete_accommodation(item_id: int, db: Session = Depends(get_db)):
